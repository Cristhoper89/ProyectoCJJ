...
class CategoriaService:
    ...
    async def cambiar_estado_categoria(self, categoria_id: int, new_state: bool) -> dict:
        logger.info(
            f"Intentando cambiar el estado de la categoría ID: {categoria_id} a {new_state}"
        )
        ...
        if categoria["estado"] is new_state:
            detalle = (
                "La categoría ya está desactivada."
                if new_state is False
                else "La categoría ya está activa."
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=detalle
        ...
        try:
            ...
            result = await self.db.execute(
                query,
                {"id": categoria_id, "new_state": new_state}
            )
            ...
            logger.error(
                f"Error al cambiar estado de categoría: {str(e)}"
            )
